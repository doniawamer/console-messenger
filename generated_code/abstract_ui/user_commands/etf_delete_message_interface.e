...
feature {NONE} -- Initialization
	...
	make(an_etf_cmd_name: STRING; etf_cmd_args: TUPLE; an_etf_cmd_container: ETF_ABSTRACT_UI_INTERFACE)
		do
			...
			etf_cmd_routine := agent delete_message(? , ?)
			etf_cmd_routine.set_operands (etf_cmd_args)
			if
				attached {INTEGER_64} etf_cmd_args[1] as uid and then attached {INTEGER_64} etf_cmd_args[2] as mid
			then
				out := "delete_message(" + etf_event_argument_out("delete_message", "uid", uid) + "," + etf_event_argument_out("delete_message", "mid", mid) + ")"
			else
				etf_cmd_error := True
			end
		end
...
feature -- command precondition 
	delete_message_precond(uid: INTEGER_64 ; mid: INTEGER_64): BOOLEAN
		do  
			Result := 
